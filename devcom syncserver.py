...
#Could not implement it properly
    def onMessage(self, data: dict):
        ...
        if data['type'] == 'probe':
            # To handle probe request
            index = data.get('from', 0)
            newdata = self.newrec[index:]
            return {'type': 'update', 'from': index, 'data': newdata}

        elif data['type'] == 'record':
            # If its already recorded
            return None

        else:
            SyncService.onMessage(data: dict)
